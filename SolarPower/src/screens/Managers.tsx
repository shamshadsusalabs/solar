...
const Managers: React.FC = () => {
    ...
    // Manager list from store (with caching)
    const {
        managerList,
        managerListLoading: loading,
        managerListError: error,
        initManagerListFromStorage,
        fetchManagerList,
    } = useManagerAuthStore();
    ...
    useEffect(() => {
        initManagerListFromStorage(); // Load cached data immediately
    }, []);

    useEffect(() => {
        if (adminAccessToken) {
            fetchManagerList(adminAccessToken); // Fetch fresh data
        }
    }, [adminAccessToken]);
    ...
    const handleRegister = async () => {
        if (!email.trim() || !name.trim() || !phoneNumber.trim() || !password.trim()) {
            Alert.alert("Missing fields", "Please fill all fields to register manager.");
            return;
        }
        ...
        try {
            ...
            // Refresh managers list from store
            fetchManagerList(adminAccessToken);
        } catch (err: any) {
            Alert.alert("Error", err?.message || "Failed to register manager.");
        } finally {
            setSubmitting(false);
        }
    };
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            Alert.alert("Error", err?.message || "Failed to update manager");
        } finally {
    ...
    const handleDelete = (manager: any) => {
        Alert.alert(
            "Delete Manager",
            `Are you sure you want to delete ${manager.email}?`,
            [
                { text: "Cancel", style: "cancel" },
                {
                    ...
                    onPress: async () => {
                        try {
                            await deleteManagerService(manager.id, adminAccessToken);
    ...
    // Filter managers - use managerList from store
    const filteredManagers = managerList.filter((mgr: any) => {
        if (!searchQuery.trim()) return true;
        const q = searchQuery.toLowerCase();
        return (
            mgr.email.toLowerCase().includes(q) ||
            mgr.phoneNumber.includes(q)
        );
    });
            ...
            <View className="mb-4 flex-row items-center justify-between">
                <View>
                    ...
                    <Text className="text-slate-500 mt-1 text-sm">
                        Manage managers & their access credentials.
                    </Text>
                </View>
                    ...
                    <View className="flex-row items-center justify-between mb-3">
                        <View>
                            ...
                            <Text className="text-[11px] text-slate-400 mt-0.5">
                                Create a new manager account with email & phone.
                            </Text>
                        </View>
            ...
            <View className="bg-white rounded-2xl p-4 mb-4 shadow-sm border border-emerald-50 flex-row justify-between items-center">
                ...
                <View className="flex-row items-center gap-x-3">
                    <View className="items-center">
                        ...
                        <Text className="text-[10px] text-slate-400">Managers</Text>
                    </View>
                    <View className="items-center">
                        <View className="h-8 w-8 rounded-full bg-emerald-50 items-center justify-center">
            ...
            {filteredManagers.length === 0 && !loading ? (
                <View className="bg-white rounded-2xl border border-emerald-100 shadow-sm px-3 py-4">
                    <Text className="text-[12px] text-slate-500">
                        No managers found. Register a new one to get started.
                    </Text>
                </View>
            ) : (
                <View className="gap-y-2">
