// controllers/chiefDashboard.controller.js
import mongoose from "mongoose";
import Lead from "../models/lead.model.js";

export const getChiefDashboard = async (req, res) => {
    try {
        const chiefId = req.user?.id || req.user?._id;

        if (!chiefId) {
            return res.status(401).json({
                success: false,
                message: "Unauthorized: chief id not found in token",
            });
        }

        // Show ALL leads (no filtering by chiefId)
        ...
        return res.status(500).json({
            success: false,
            message: "Failed to load chief dashboard",
        });
    }
};
